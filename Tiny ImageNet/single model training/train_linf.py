...
def train_clean (train_loader, device,optimizer,model,CE_loss):
    print('doing clean training now')
...
def val(val_loader, device, model,  basic_model, AttackPGD, config_l2, config_linf, attack,\
    ...
    print('doing clean images test')
    for images_clean, labels in val_loader:
    ...
    print('doing l2 attacked images test')
    
    for images_l2, labels in tqdm(val_loader):
            ...
            images_l2 = images_l2.to(device)
            labels = labels.to(device)
            
            images_att = net_attack(images_l2,labels, attack)
    ...
    print('doing linf adv test')

    for images_linf, labels in tqdm(val_loader):
            ...
            #print('processing testing image:', a)
            #images, labels = utils.cuda([images, labels], args.gpu_ids)
            images_linf = images_linf.to(device)
            labels = labels.to(device)
            
            images_att = net_attack(images_linf,labels, attack)
...
def train(args):
    ...
    for i in range(args.epochs):
       ...
       # train_clean (train_loader, device,optimizer,model,CE_loss)
        ...
        acc_nat, best_acc_nat, acc_l2, best_acc_l2, acc_linf, best_acc_linf = val(val_loader, device, model,  model, AttackPGD, config_l2, config_linf, attack,\
